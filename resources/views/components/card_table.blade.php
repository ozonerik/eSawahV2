                    ...
                    <tbody>
                        ...
                        <tr class="@if($this->is_checked($row->id)) table-primary @endif">
                            ...
                            <td>
                                ...
                                <button wire:click.prevent="onEdit({{ $row->id }})" class="btn btn-sm btn-success w-50" data-toggle="tooltip" title="Edit" ><i class="fas fa-edit"></i></button>         
                                <button wire:click.prevent="onDelete({{ $row->id }})" class="btn btn-sm btn-danger w-50 mt-1" data-toggle="tooltip" title="Hapus"><i class="fas fa-trash-alt"></i></button>
                                @elseif(in_array('edit',$tbtn))
                                <button wire:click.prevent="onEdit({{ $row->id }})" class="btn btn-sm btn-success w-50" data-toggle="tooltip" title="Edit" ><i class="fas fa-edit"></i></button>
                                @elseif(in_array('del',$tbtn))
                                <button wire:click.prevent="onDelete({{ $row->id }})" class="btn btn-sm btn-danger w-50" data-toggle="tooltip" title="Hapus"><i class="fas fa-trash-alt"></i></button>
                                @endif
                            </td>
                        </tr>
                        @endforeach
